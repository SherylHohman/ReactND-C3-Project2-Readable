// TEMP: New Thoughts:
// Treat router *as-if* it *is* store.
// (don't actually save anything from router to store though - NEVERMIND:SEE BELOW)
// just have selectors that return its values

// if route is EXACT match, return router version
// if NOT exact match, return last saved value from store
// (ok, still need to save the data to store then)

// ? ASKING for router version then sends the CHANGE_VIEW
//  call (instead of waiting for cDM to call CHANGE_VIEW)
//  Does this make sense? Is it possible?
//  Does this provide any benefits? (ie, save a render cycle?)

// Hmm.. categories now relies on url, so it doesn't care about stored loc anymore.
// (which it had to do when it relied on the route due to (match.route))
// now it has a reverse lookup to get the actual PAGE/URL route.. ..
// Perhaps Don't need to ever store loc after all
// - just provide memoised selectors to return  loc-like object.
// - and individual loc properties.

// Then viewData store only handles sort-by and (sort-order).
